...
Ext.onReady(function() {
		...
		/*{
				...
				// hidden : true,
				width		: 100,
				renderer	: HEATTREATRender,
				sortable	: true
			}, {
				header		: '毛坯状态',
				dataIndex	: 'blanktype',
				width		: 160,
				renderer	: BLANKTYPERender,
				// hidden : true,
				sortable	: true
			}, {
				header		: '硬度',
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'workpiecematerial.do?code=queryWorkpiecematerial4Manage'
								// queryWorkpiecematerial4Manage是在WorkpiecematerialAction中的方法queryWorkpiecematerial4Manage;
							}),
			...
			var heattreatcodeCombo = new Ext.form.ComboBox({
						fieldLabel		: '热处理代码',
						hiddenName		: 'heattreatcode',
						id				: 'heattreatcodeid',
						emptyText		: '请选择热处理代码...',
						triggerAction	: 'all',
						store			: HEATTREATCODEStore,
						displayField	: 'text',
						valueField		: 'value',
						loadingText		: '正在加载数据...',
						...
						listeners		: {
							focus	: function() {
								...
								setTimeout('Ext.getCmp("heattreatcodeid").expand();', 1);
							}/*
								 * , select : function(e) { var v = e.getValue(); // 自动识别类型(热处理代码与热处理类型之间对应关系); if (v == '0') { Ext.getCmp('heattreatid').setValue(''); } else if (v == '1') {
								 * Ext.getCmp('heattreatid').setValue(''); } else if (v == '2') { Ext.getCmp('heattreatid').setValue(''); } else if (v == '3') { Ext.getCmp('heattreatid').setValue(''); }
								 * else if (v == '4') { Ext.getCmp('heattreatid').setValue(''); } else if (v == '5') { Ext.getCmp('heattreatid').setValue('调质'); } else if (v == '6') {
								 * Ext.getCmp('heattreatid').setValue('渗碳'); } }
								 */
						}
					});

			var blanktypeCombo = new Ext.form.ComboBox({
						fieldLabel		: '毛坯状态',
						hiddenName		: 'blanktype',
						id				: 'blanktypeid',
						emptyText		: '请选择毛坯状态...',
						triggerAction	: 'all',
						store			: BLANKTYPEStore,
						displayField	: 'text',
						valueField		: 'value',
						loadingText		: '正在加载数据...',
						mode			: 'local', // 数据会自动读取,如果设置为local又调用了store.load()则会读取2次；也可以将其设置为local，然后通过store.load()方法来读取
						...
						listeners		: {
							focus	: function() {
								// bladematerialStore.load();
								setTimeout('Ext.getCmp("blanktypeid").expand();', 1);
							}
						}
					});

			var heattreatCombo = new Ext.form.ComboBox({
						fieldLabel		: '热处理状态',
						hiddenName		: 'heattreat',
						id				: 'heattreatid',
						emptyText		: '请选择热处理状态...',
						triggerAction	: 'all',
						store			: HEATTREATStore,
						displayField	: 'text',
						valueField		: 'value',
						loadingText		: '正在加载数据...',
						mode			: 'local', // 数据会自动读取,如果设置为local又调用了store.load()则会读取2次；也可以将其设置为local，然后通过store.load()方法来读取
						...
						listeners		: {
							focus	: function() {
								// bladematerialStore.load();
								setTimeout('Ext.getCmp("heattreatid").expand();', 1);
							}
						}
					});
			...
			var addItemForm = new Ext.form.FormPanel({
						...
						items		: [{
									...
									items	: [{
																...
																blur	: function(e) {
																	if (!isUndefined(e.getValue())) {
																		e.setValue(e.getValue().toUpperCase());
																				 ...
																				 * { fieldLabel : '热处理代码', xtype : 'numberfield', name : 'heattreatcode', emptyText : '请输入热处理代码...', listeners : { blur :
																				 * function(e) { var v = e.getValue(); // 自动识别类型(热处理代码与热处理类型之间对应关系); if (v == '0') {
																				 * Ext.getCmp('heattreatid').setValue(''); } else if (v == '1') { Ext.getCmp('heattreatid').setValue(''); } else if (v ==
																				 * '2') { Ext.getCmp('heattreatid').setValue(''); } else if (v == '3') { Ext.getCmp('heattreatid').setValue(''); } else
																				 * if (v == '4') { Ext.getCmp('heattreatid').setValue(''); } else if (v == '5') { Ext.getCmp('heattreatid')
																				 * .setValue('调质'); } else if (v == '6') { Ext.getCmp('heattreatid') .setValue('渗碳'); } } } }
																				 */, {
															fieldLabel	: '材料类型',
															name		: 'wmtype',
															emptyText	: '请输入材料类型...'
												...
												items		: [/*
																 * { fieldLabel : '毛坯状态', name : 'blanktype', emptyText : '请输入毛坯状态...' }
																 */blanktypeCombo, {
															fieldLabel	: '硬度',
															name		: 'hardness',
															emptyText	: '请输入硬度...'
																...
																blur	: function(e) {
																	if (!isUndefined(e.getValue())) {
																		e.setValue(e.getValue().toUpperCase());
																			 ...
																			 * { fieldLabel : '热处理状态', name : 'heattreat', id : 'heattreatid', emptyText : '请输入热处理状态...' }
																			 */]
											}]
								}, {
									fieldLabel	: '用途',
			...
			var addItem = new Ext.Window({
						...
						layout		: 'fit',
						width		: 800,
						height		: Ext.isChrome ? 243 : (Ext.isFirefox ? 239 : 239),
						// height : 265,
						modal		: true,
						closable	: true, // 是否可关闭
						closeAction	: 'hide',
						...
						// pageY : document.body.clientHeight / 2 - 240 / 2,//pageY : 20, // 页面定位Y坐标
			...
			var grid = new Ext.grid.GridPanel({
						title		: '<span class="commoncss">工件材料</span>',
						header		: false,
						height		: 500,
						autoScroll	: true,
						frame		: true,
						...
						plugins		: [filters],
						stateful	: sf,
						stateId		: 'grid-workpiece-material'
					});
			...
			function editInit() {
				...
				Ext.getCmp('workpiecematerialid').setValue(record.get('workpiecematerialid'));
				addItem.show();
			}
			...
			function deleteInit() {
				...
				var strChecked = jsArray2JsString(records, 'workpiecematerialid');
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'workpiecematerial.do?code=deleteWorkpiecematerial',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url				: 'workpiecematerial.do?code=saveWorkpiecematerial',
							waitTitle		: '提示',
							method			: 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url				: 'workpiecematerial.do?code=updateWorkpiecematerial',
							waitTitle		: '提示',
							method			: 'POST',
							submitEmptyText	: false,
							waitMsg			: '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'workpiecematerial.do?code=importWorkpiecematerialFromExcel',
															waitTitle : '提示',
															method : 'POST',
