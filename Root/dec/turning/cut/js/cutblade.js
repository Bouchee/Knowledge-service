...
Ext.onReady(function() {
			...
			var cm = new Ext.grid.ColumnModel([rownum, sm, {
				...
				sortable	: true
					/*
					 * , renderer : SIDERELIEFANGLERender
					 */
				}, {
				header		: '最大切槽深度',
				 ...
				 * { header : '优先级', dataIndex : 'priority', width : 70, align : 'center', //hidden : true, sortable : true,
				 *  },
				 */{
				header		: '最大切削深度',
				dataIndex	: 'apmax',
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'cutblade.do?code=queryCutblade4Manage'
			...
			var cutbladematerialStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'cutblade.do?code=queryCutbladematerial4Cutblade'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			cutbladematerialStore.load(); // 如果mode : 'local',时候才需要手动load();

			var cutbladematerialCombo = new Ext.form.ComboBox({
						fieldLabel		: '刀片材质',
						hiddenName		: 'bladematerialid',
						id				: 'bladematerialidid',
						emptyText		: '请选择刀片材质...',
						triggerAction	: 'all',
						store			: cutbladematerialStore,
						displayField	: 'text',
						valueField		: 'value',
						loadingText		: '正在加载数据...',
			...
			var grooveStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'cutblade.do?code=queryGroove4Cutblade'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			var priorityStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'cutblade.do?code=queryPriority4Cutblade'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			priorityStore.load(); // 如果mode : 'local',时候才需要手动load();
			...
			var priorityCombo = new Ext.form.ComboBox({
				...
				listeners		: {
					focus	: function() {
						// brandStore.load();
						setTimeout('Ext.getCmp("priorityidid").expand();', 1);
					}
				}
			});
			...
			var brandStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'cutblade.do?code=queryBrand4Cutblade'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			/*
			 * var bladewidthStore = new Ext.data.Store({ proxy : new Ext.data.HttpProxy({ url : 'cutblade.do?code=queryBladewidth4Cutblade' }), reader : new Ext.data.JsonReader({ totalProperty :
			 * 'TOTALCOUNT', root : 'ROOT' }, [{ name : 'value' }, { name : 'text' }]), listeners : { // 设置远程数据源下拉选择框的初始值 'load' : function(obj) { // classStore.setValue('530101'); } } });
			 * bladewidthStore.load(); // 如果mode : 'local',时候才需要手动load();
			 * 
			 * var bladewidthCombo = new Ext.form.ComboBox({ fieldLabel : '刀片宽度', hiddenName : 'bladewidth', id : 'bladewidthidid', emptyText : '请输入刀片宽度..', triggerAction : 'all', store :
			 * BLADEWIDTHStore, displayField : 'text', valueField : 'value', loadingText : '正在加载数据...', mode : 'local', //editable : false, forceSelection : true, plugins : [new
			 * QM.plugin.PinyinFilter], resizable : true, anchor : '94.6%', listeners : { focus : function() { setTimeout('Ext.getCmp("bladewidthidid").expand();', 1); } } });
			 * 
			 * 
			 * 
			 * var mainangleCombo = new Ext.form.ComboBox({ fieldLabel : '主偏角', hiddenName : 'mainangle', id : 'mainangleidid', emptyText : '请输入主偏角..', triggerAction : 'all', store : MAINANGLEStore,
			 * displayField : 'text', valueField : 'value', loadingText : '正在加载数据...', mode : 'local', //editable : false, forceSelection : true, plugins : [new QM.plugin.PinyinFilter], resizable :
			 * true, anchor : '94.6%', listeners : { focus : function() { setTimeout('Ext.getCmp("mainangleidid").expand();', 1); } } });
			 * 
			 * var reliefangleCombo = new Ext.form.ComboBox({ fieldLabel : '刀片后角', hiddenName : 'reliefangle', id : 'reliefangleidid', emptyText : '请输入刀片后角..', triggerAction : 'all', store :
			 * RELIEFANGLEStore, displayField : 'text', valueField : 'value', loadingText : '正在加载数据...', mode : 'local', //editable : false, forceSelection : true, plugins : [new
			 * QM.plugin.PinyinFilter], resizable : true, anchor : '94.6%', listeners : { focus : function() { setTimeout('Ext.getCmp("reliefangleidid").expand();', 1); } } });
			 * 
			 * var sidereliefangleCombo = new Ext.form.ComboBox({ fieldLabel : '刀片侧后角', hiddenName : 'sidereliefangle', id : 'sidereliefangleidid', emptyText : '请输入刀片侧后角..', triggerAction : 'all',
			 * store : SIDERELIEFANGLEStore, displayField : 'text', valueField : 'value', loadingText : '正在加载数据...', mode : 'local', //editable : false, forceSelection : true, plugins : [new
			 * QM.plugin.PinyinFilter], resizable : true, anchor : '94.6%', listeners : { focus : function() { setTimeout('Ext.getCmp("sidereliefangleidid").expand();', 1); } } });
			 * 
			 * var cuttingdepthCombo = new Ext.form.ComboBox({ fieldLabel : '最大切槽深度', hiddenName : 'cuttingdepth', id : 'cuttingdepthidid', emptyText : '请输入最大切槽深度..', triggerAction : 'all', store :
			 * CUTTINGDEPTHStore, displayField : 'text', valueField : 'value', loadingText : '正在加载数据...', mode : 'local', //editable : false, forceSelection : true, plugins : [new
			 * QM.plugin.PinyinFilter], resizable : true, anchor : '94.6%', listeners : { focus : function() { setTimeout('Ext.getCmp("cuttingdepthidid").expand();', 1); } } });
			 * 
			 * var cuttingradiusCombo = new Ext.form.ComboBox({ fieldLabel : '圆角半径', hiddenName : 'cuttingradius', id : 'cuttingradiusidid', emptyText : '请输入圆角半径..', triggerAction : 'all', store :
			 * CUTTINGRADIUSStore, displayField : 'text', valueField : 'value', loadingText : '正在加载数据...', mode : 'local', //editable : false, forceSelection : true, plugins : [new
			 * QM.plugin.PinyinFilter], resizable : true, anchor : '94.6%', listeners : { focus : function() { setTimeout('Ext.getCmp("cuttingradiusidid").expand();', 1); } } });
			 */
						...
						items		: [{
									...
									items	: [{
												 ...
												 */
															...
															name		: 'name',
															emptyText	: '请输入刀片名称...'
														}, brandCombo, turningtypeCombo, cuttinghandCombo, cuttingedgeCombo, cutbladematerialCombo, {
															fieldLabel	: '刀片宽度',
															name		: 'bladewidth',
															emptyText	: '请输入刀片宽度...'
														},{
			...
			var addItem = new Ext.Window({
						...
						layout		: 'fit',
						width		: 800,
						height		: Ext.isChrome ? 559 : (Ext.isFirefox ? 555 : 555),
						// height : 470,
						modal		: true,
						closable	: true, // 是否可关闭
						closeAction	: 'hide',
			...
			function editInit() {
				...
				/*
				 * var da2 = record.data.figure2; imageshowWin2 = imageshow(da2); if (imageshowWin2 != null) { imageshowWin2.show(); } else { infoMsg('该条目还没有添加图片!'); }
				 */
				...
				Ext.getCmp('cutbladeid').setValue(record.get('cutbladeid'));
			...
			function deleteInit() {
				...
				var strChecked = jsArray2JsString(records, 'cutbladeid');
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'cutblade.do?code=deleteCutblade',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url				: 'cutblade.do?code=saveCutblade',
							waitTitle		: '提示',
							method			: 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url				: 'cutblade.do?code=updateCutblade',
							waitTitle		: '提示',
							method			: 'POST',
							submitEmptyText	: false,
							waitMsg			: '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'cutblade.do?code=importCutbladeFromExcel',
															waitTitle : '提示',
															method : 'POST',
