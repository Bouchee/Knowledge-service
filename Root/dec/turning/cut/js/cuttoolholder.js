...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'cuttoolholder.do?code=queryCuttoolholder4Manage'
			...
			var brandStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'cuttoolholder.do?code=queryBrand4Cuttoolholder'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
						...
						items		: [{
									...
									items	: [{
												...
												items		: [{
															...
															name		: 'name',
															emptyText	: '请输入刀体名称...'
														}, brandCombo, turningtypeCombo, cseriesCombo]
											}, {
												columnWidth	: .33,
												layout		: 'form',
												defaultType	: 'textfield',
			...
			var addItem = new Ext.Window({
						...
						layout			: 'fit',
						width			: 800,
						height			: Ext.isChrome ? 655 : (Ext.isFirefox ? 651 : 572),
						// height : 650,
						closable		: true, // 是否可关闭
						closeAction		: 'hide',
						animCollapse	: true,
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'cuttoolholder.do?code=deleteCuttoolholder',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url				: 'cuttoolholder.do?code=saveCuttoolholder',
							waitTitle		: '提示',
							method			: 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url				: 'cuttoolholder.do?code=updateCuttoolholder',
							waitTitle		: '提示',
							method			: 'POST',
							submitEmptyText	: false,
							waitMsg			: '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'cuttoolholder.do?code=importCuttoolholderFromExcel',
															waitTitle : '提示',
															method : 'POST',
