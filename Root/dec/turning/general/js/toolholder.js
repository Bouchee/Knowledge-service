...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'toolholder.do?code=queryToolholder4Manage'
			...
			var brandStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'toolholder.do?code=queryBrand4Toolholder'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			var addItem = new Ext.Window({
						...
						layout			: 'fit',
						width			: 800,
						height			: Ext.isChrome ? 603 : (Ext.isFirefox ? 599 : 599),
						// height : 600,
						closable		: true, // 是否可关闭
						closeAction		: 'hide',
						animCollapse	: true,
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'toolholder.do?code=deleteToolholder',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url				: 'toolholder.do?code=saveToolholder',
							waitTitle		: '提示',
							method			: 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url				: 'toolholder.do?code=updateToolholder',
							waitTitle		: '提示',
							method			: 'POST',
							submitEmptyText	: false,
							waitMsg			: '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'toolholder.do?code=importToolholderFromExcel',
															waitTitle : '提示',
															method : 'POST',
