...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
									url	: 'cutbladematerial.do?code=queryCutbladematerial4Manage'
								}),
			...
			var brandStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'cutbladematerial.do?code=queryBrand4Cutbladematerial'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			var grid = new Ext.grid.GridPanel({
						...
						plugins		: [filters],
						stateful	: sf,
						stateId		: 'grid-cut-blade-material'
					});
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'cutbladematerial.do?code=deleteCutbladematerial',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url				: 'cutbladematerial.do?code=saveCutbladematerial',
							waitTitle		: '提示',
							method			: 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url				: 'cutbladematerial.do?code=updateCutbladematerial',
							waitTitle		: '提示',
							method			: 'POST',
							submitEmptyText	: false,
							waitMsg			: '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'cutbladematerial.do?code=importCutbladematerialFromExcel',
															waitTitle : '提示',
															method : 'POST',
