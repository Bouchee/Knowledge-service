...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'bladematerial.do?code=queryBladematerial4Manage'
								// queryBladematerial4Manage是在BladematerialAction中的方法queryBladematerial4Manage;
							}),
			...
			var brandStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'bladematerial.do?code=queryBrand4Bladematerial'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'bladematerial.do?code=deleteBladematerial',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url				: 'bladematerial.do?code=saveBladematerial',
							waitTitle		: '提示',
							method			: 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url				: 'bladematerial.do?code=updateBladematerial',
							waitTitle		: '提示',
							method			: 'POST',
							submitEmptyText	: false,
							waitMsg			: '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'bladematerial.do?code=importBladematerialFromExcel',
															waitTitle : '提示',
															method : 'POST',
