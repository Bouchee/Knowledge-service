...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
				proxy	: new Ext.data.HttpProxy({
					url	: 'hd_toolholder_reaming_integral.do?code=queryHd_toolholder_reaming_integral4Manage'
						  
						// queryHd_toolholder_reaming_integral4Manage是在Hd_toolholder_reaming_integralAction中的方法queryHd_toolholder_reaming_integral4Manage;
					}),
	...
	var brandStore = new Ext.data.Store({
				proxy		: new Ext.data.HttpProxy({
							url	: 'hd_toolholder_reaming_integral.do?code=queryBrand4Hd_toolholder_reaming_integral'
						}),
				reader		: new Ext.data.JsonReader({
							totalProperty	: 'TOTALCOUNT',
							root			: 'ROOT'
	...
	var grid = new Ext.grid.GridPanel({
		title		: '<span class="commoncss">整体刀具</span>',
		header		: false,
		height		: 500,
		autoScroll	: true,
		frame		: true,
		...
		plugins		: [filters],
		stateful	: sf,
		stateId		: 'grid-hd_toolholder_reaming_integral'
			/*
	...
	function editInit() {
		...
		imageshowWin = imageshow2(record.data.figure, record.data.figure2, addItem, 0);
		if (imageshowWin != null) {
			imageshowWin.show();
		} else {
			infoMsg('该条目还没有添加图片!');
	 ...
	 * 
	 * if (path2 == null || path2 == '') { path2 = '\\resource\\image\\ext\\null.gif'; } var readWin = new Ext.Window({ title : '整体刀具性能卡及简图', width : 800, modal : true, height : 500, closeAction :
	 ...
	 * autoEl : { tag : 'iframe', style : 'height: 100%; width: 100%', src : webContext + path1 } }, { xtype : 'box', title : '整体刀具简图', autoEl : { tag : 'iframe', style : 'height: 100%; width: 100%',
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url		: 'hd_toolholder_reaming_integral.do?code=deleteHd_toolholder_reaming_integral',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
					url				: 'hd_toolholder_reaming_integral.do?code=saveHd_toolholder_reaming_integral',
					waitTitle		: '提示',
					method			: 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
					url				: 'hd_toolholder_reaming_integral.do?code=updateHd_toolholder_reaming_integral',
					waitTitle		: '提示',
					method			: 'POST',
					submitEmptyText	: false,
					waitMsg			: '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_toolholder_reaming_integral.do?code=importHd_toolholder_reaming_integralFromExcel',
													waitTitle : '提示',
													method : 'POST',
