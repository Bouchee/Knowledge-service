...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_toolholder_reaming_clamp.do?code=queryHd_toolholder_reaming_clamp4Manage'
				// queryHd_toolholder_boring4Manage是在Hd_toolholder_boringAction中的方法queryHd_toolholder_boring4Manage;
		}),
	...
	var brandStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_toolholder_reaming_clamp.do?code=queryBrand4Hd_toolholder_reaming_clamp'
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
	...
	var addItemForm = new Ext.form.FormPanel({
				...
				items : [{
					 ...
					 * { xtype : 'fieldset', collapsible : true, collapsed :
					 * true, title : '附件信息', layout : 'fit', id : 'fieldset1',
					 * animCollapse : true, height : 50, defaults : { anchor :
					 * '100%' }, listeners : { 'collapse' : function(obj) {
					 * addItem.setHeight(addItem.getHeight() - 160);
					 * addItem.setPagePosition(document.body.clientWidth / 2 -
					 * 800 / 2, document.body.clientHeight / 2 -
					 * addItem.getHeight() / 2); addItem.doLayout(false, true); //
					 * Ext.getCmp('fieldset1').expand(); }, 'expand' :
					 * function(obj) { addItem.setHeight(addItem.getHeight() +
					 * 160); addItem.setPagePosition(document.body.clientWidth /
					 * 2 - 800 / 2, document.body.clientHeight / 2 -
					 * addItem.getHeight() / 2); addItem.doLayout(false, true); //
					 * Ext.getCmp('fieldset1').collapse(); } }, height : '100%',
					 * items : [{ layout : 'column', items : [{ columnWidth :
					 * .33, layout : 'form', defaultType : 'textfield', defaults : {
					 * width : '85%', anchor : '89%', maxLength : mlm,
					 * allowBlank : true, labelStyle : fucolor }, items : [{
					 * fieldLabel : '附件1名称', name : 'aname1', emptyText :
					 * '请输入附件1名称...' }, { fieldLabel : '附件1型号', name : 'atype1',
					 * emptyText : '请输入附件1型号...' }, { fieldLabel : '附件2名称', name :
					 * 'aname2', emptyText : '请输入附件2名称...' }, { fieldLabel :
					 * '附件2型号', name : 'atype2', emptyText : '请输入附件2型号...' }, {
					 * fieldLabel : '附件3名称', name : 'aname3', emptyText :
					 * '请输入附件3名称...' }, { fieldLabel : '附件3型号', name : 'atype3',
					 * emptyText : '请输入附件3型号...' }] }, { columnWidth : .33,
					 * layout : 'form', defaultType : 'textfield', defaults : {
					 * width : '85%', anchor : '89%', maxLength : mlm,
					 * labelStyle : fucolor, allowBlank : true }, items : [{
					 * fieldLabel : '附件4名称', name : 'aname4', emptyText :
					 * '请输入附件4名称...' }, { fieldLabel : '附件4型号', name : 'atype4',
					 * emptyText : '请输入附件4型号...' }, { fieldLabel : '附件5名称', name :
					 * 'aname5', emptyText : '请输入附件5名称...' }, { fieldLabel :
					 * '附件5型号', name : 'atype5', emptyText : '请输入附件5型号...' }, {
					 * fieldLabel : '附件6名称', name : 'aname6', emptyText :
					 * '请输入附件6名称...' }, { fieldLabel : '附件6型号', name : 'atype6',
					 * emptyText : '请输入附件6型号...' }] }, { columnWidth : .33,
					 * layout : 'form', defaultType : 'textfield', defaults : {
					 * width : '85%', anchor : '89%', maxLength : mlm,
					 * labelStyle : fucolor, allowBlank : true }, items : [{
					 * fieldLabel : '附件7名称', name : 'aname7', emptyText :
					 * '请输入附件7名称...' }, { fieldLabel : '附件7型号', name : 'atype7',
					 * emptyText : '请输入附件7型号...' }, { fieldLabel : '附件8名称', name :
					 * 'aname8', emptyText : '请输入附件8名称...' }, { fieldLabel :
					 * '附件8型号', name : 'atype8', emptyText : '请输入附件8型号...' }, {
					 * fieldLabel : '附件9名称', name : 'aname9', emptyText :
					 * '请输入附件9名称...' }, { fieldLabel : '附件9型号', name : 'atype9',
					 * emptyText : '请输入附件9型号...' }] }] }] },
					 */{
					id : 'toolholderid',
					name : 'toolholderid',
	...
	var addItem = new Ext.Window({
		...
		layout : 'fit',
		width : 800,
		height : Ext.isChrome ? 400 : (Ext.isFirefox ? 597 : 597),
		// height : Ext.isChrome ? 380 : (Ext.isFirefox ? 380 :
		// 400),
		modal : false,
		closable : true, // 是否可关闭
		closeAction : 'hide',
		...
		// 2,//pageY : 20, //
		// 页面定位Y坐标
		// x : document.body.clientWidth / 2 - 1000 / 2, //
		// 页面定位X坐标
		items : [addItemForm],
		buttons : [{
					text : '保存',
	...
	var grid = new Ext.grid.GridPanel({
		title : '<span class="commoncss">机夹刀具刀体</span>',
		header : false,
		height : 500,
		autoScroll : true,
		frame : true,
		...
		// plugins : [ filters ],
		stateful : sf,
		stateId : 'grid-hd_toolholder_reaming_clamp'
			/*
	...
	function editInit() {
		...
		// imageshowWin = imageshow2(record.data.figure, "", addItem,
		// 0);
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'hd_toolholder_reaming_clamp.do?code=deleteHd_toolholder_reaming_clamp',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
			url : 'hd_toolholder_reaming_clamp.do?code=saveHd_toolholder_reaming_clamp',
			waitTitle : '提示',
			method : 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
			url : 'hd_toolholder_reaming_clamp.do?code=updateHd_toolholder_reaming_clamp',
			waitTitle : '提示',
			method : 'POST',
			submitEmptyText : false,
			waitMsg : '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_toolholder_reaming_clamp.do?code=importHd_toolholder_reaming_clampFromExcel',
													waitTitle : '提示',
													method : 'POST',
