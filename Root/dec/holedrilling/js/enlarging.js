...
Ext
		.onReady(function() {
			...
			// 扩孔 enlarging Enlarging
			// 被加工材料 workpiecematerial Workpiecematerial
			// 直径 nominaldiameter Nominaldiameter NOMINALDIAMETER
			// 深度 depth Depth
			// 底孔径 porediamete Porediamete
			...
			// 列模型（扩孔）
			var enlargingCm = new Ext.grid.ColumnModel([ rownum, sm, {
				header : 'SAP编号',
				dataIndex : 'sapcode',
				width : 80,
				hidden : false,
			...
			// Store数据存储
			var enlargingStore = new Ext.data.GroupingStore({
				proxy : new Ext.data.HttpProxy({
					url : 'enlarging.do?code=queryHd_holedrilling4Manage'
				}),
				...
				enlargingStore
						.load({
							params : {
								...
								porediamete : porediameteCombo.getValue(),
								name : tooltypeCombo.getValue()
							}
						});
			}
			...
			var workpieceisotypeStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'enlarging.do?code=queryWorkpiecematerial'
				}),
				reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
			...
			var nominaldiameterStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'enlarging.do?code=queryNominaldiameter'
				}),
				reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
			...
			var gongchengzhijinDisplayWin = new Ext.Window(
					{
						...
						x:((document.body.clientWidth)/numlign-divnum)*2+135,
						...
						closable:false,
					     ...
					     items: [ {
								...
								autoEl : {
									...
									src : 'dec/combo/alter/new/kuokong_gongchengzhijin.png'
								}
							} 
						...
						listeners : {
							...
							specialkey : function(c, e) {
								...
								if (e.getKey() == e.ENTER) {
									c
											.setValue(Ext
													.get('nominaldiameteridid').dom.value);
									refreshEnlarging();
								}
							}
						}
			...
			var depthStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'enlarging.do?code=queryDepth'
				}),
				reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
			...
			//提示框
			var kuokongshenduDisplayWin = new Ext.Window(
					{
						//title : '选择助手',
						id:'kuokongshenduDisplayWinid',
						...
						closable:false,
					     ...
					     items: [ {
								...
								autoEl : {
									tag : 'img',
									style : 'width:100%;height:100%;' ,
									src : 'dec/combo/alter/new/kuokong_zuankongshendu.png'
								}
							} 
			...
			var depthCombo = new Ext.form.ComboBox({
				...
				listeners : {
					...
					expand:function(){kuokongshenduDisplayWin.show()},
					collapse:function(){kuokongshenduDisplayWin.hide()},
					/*
					 * focus : function() { if (parent.rhwf) { hp =
					 * helpWin('test'); }
			...
			// 底孔径数据
			var porediameteStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'enlarging.do?code=queryPorediamete'
				}),
				reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
			...
			porediameteStore.load();

			// 底孔径下拉框
			var porediameteCombo = new Ext.form.ComboBox({
				fieldLabel : '底孔径',
				hiddenName : 'porediameteid',
				id : 'porediameteidid',
				emptyText : '请选择...',
				triggerAction : 'all',
				store : porediameteStore,
				displayField : 'value',
				valueField : 'value',
				selectOnFocus : true,
				loadingText : '正在加载数据...',
				...
				listeners : {
					 ...
					 * focus : function() { if (parent.rhwf) { hp =
					 * helpWin('test'); }
					 * setTimeout('Ext.getCmp("threadspecificationidid").expand();',
					 * 10); }, blur : function() { if (Ext.isDefined(hp) &&
					 * hp.isVisible()) { hp.close(); } },
					 */
					specialkey : function(c, e) {
						...
						if (e.getKey() == e.ENTER) {
							c.setValue(Ext.get('porediameteidid').dom.value);
							refreshEnlarging();
						}
					}
				}
			...
			var tooltypeStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'enlarging.do?code=queryTooltypeInhd_holedrilling'
				}),
				reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
			...
			var tbar = new Ext.Toolbar({
				items : [ new Ext.Toolbar.TextItem({
					...
					text : '底孔径:',
					autoHeight : true
				}), porediameteCombo, '-', new Ext.Toolbar.TextItem({
					text : '刀具类型:',
					autoHeight : true
				}), tooltypeCombo, '-', '-', {
					text : '打印预览',
			...
			// 刷新刀具
			function refreshEnlarging() {
				if (parent.grf == true) {
					refreshEnlargingForce();
				}
			}

			// 强制刷新刀具
			function refreshEnlargingForce() {
				enlargingStoreLoad();
			}
			...
			function propertyDisplay() {
				...
				/*
				 * // 详情数据储存 var hd_toolholder_coredrilling_shellStore = new
				 * Ext.data.GroupingStore( { proxy : new Ext.data.HttpProxy( {
				 * url :
				 * 'enlarging.do?code=queryHd_toolholder_coredrilling_shell4Manage' //
				 * queryHd_toolholder_coredrilling_shell4Manage是在EnlargingAction中的方法queryHd_toolholder_coredrilling_shell4Manage;
				 * }), reader : new Ext.data.JsonReader({ totalProperty :
				 * 'TOTALCOUNT', root : 'ROOT' }, [ { name : 'toolholderid' }, {
				 * name : 'sapcode' }, { name : 'name' }, { name : 'type' }, {
				 * name : 'figure' }, { name : 'figure2' }, { name : 'brandid' }, {
				 * name : 'cuttype' }, { name : 'typeid' }, { name :
				 * 'hdtooltype' }, { name : 'description' }, { name : 'remark' }, {
				 * name : 'k1' }, { name : 'hdtcsid' }, { name : 'standard' }, {
				 * name : 'nominaldiameter' }, { name : 'materialid' }, { name :
				 * 'toothnumber' }, { name : 'porediamete' }, { name : 'length' }, {
				 * name : 'portsize' } , { name : 'coolingtype' } ]), listeners : { }
				 * });
				 * 
				 * hd_toolholder_coredrilling_shellStore.load();
				 */

				/*
				 * var hd_toolholder_coredrilling_shellCm = new
				 * Ext.grid.PropertyColumnModel( [ { header : '编号', // 列标题
				 * dataIndex : 'toolholderid', // 数据索引,和Store数据存储模型对应 width :
				 * 60, // 列宽度 sortable : true // 该列是否要排序 // hidden : true //
				 * 该列是否隐藏 }, { header : 'SAP编码', dataIndex : 'sapcode', width :
				 * 60, sortable : true // hidden : true }, { header : '名称',
				 * dataIndex : 'name', width : 100, sortable : true // hidden :
				 * true }, { header : '型号', dataIndex : 'type', width : 130, //
				 * hidden : true, sortable : true }, { header : '简图', dataIndex :
				 * 'figure', width : 80, // hidden : true, sortable : true }, {
				 * header : '实物图', dataIndex : 'figure2', width : 80, // hidden :
				 * true, sortable : true }, { header : '品牌', dataIndex :
				 * 'brandid', width : 80, // renderer : TURNINGTYPERender, //
				 * hidden : true, sortable : true }, { header : '切削类型',
				 * dataIndex : 'cuttype', width : 80, align : 'center', //
				 * hidden : true, sortable : true }, { header : '类型详情ID',
				 * dataIndex : 'typeid', width : 80, align : 'center', hidden :
				 * true, sortable : true }, { header : '刀具类型', dataIndex :
				 * 'hdtooltype', width : 80, align : 'center', renderer :
				 * HDTOOLTYPERender, // hidden : true, sortable : true }, {
				 * header : '特点', dataIndex : 'description', width : 100, align :
				 * 'center', // renderer : CSERIESRender, // hidden : true,
				 * sortable : true }, { header : '备注信息', dataIndex : 'remark',
				 * width : 80, // hidden : true, sortable : true }, { header :
				 * '系数K1', dataIndex : 'K1', width : 80, // hidden : true,
				 * sortable : true }, { header : '编号', dataIndex : 'hdtcsid',
				 * width : 80, // hidden : true, sortable : true }, { header :
				 * '标准', dataIndex : 'standard', width : 80, // hidden : true,
				 * sortable : true }, { header : '公称直径', dataIndex :
				 * 'nominaldiameter', width : 80, // hidden : true, sortable :
				 * true }, { header : '材质', dataIndex : 'materialid', width :
				 * 80, // hidden : true, sortable : true }, { header : '齿数',
				 * dataIndex : 'toothnumber', width : 80, // hidden : true,
				 * sortable : true }, { header : '底孔径', dataIndex :
				 * 'porediamete', width : 80, // hidden : true, sortable : true }, {
				 * header : '总长', dataIndex : 'length', width : 80, // hidden :
				 * true, sortable : true }, { header : '接口尺寸', dataIndex :
				 * 'portsize', width : 80, // hidden : true, sortable : true } , {
				 * header : '冷却类型', dataIndex : 'coolingtype', width : 80, //
				...
				var propertyGrid = new Ext.grid.PropertyGrid({
					...
					autoSort : false,
					frame : true,
					// store : hd_toolholder_coredrilling_shellStore,
					// store : enlargingStore,
					id : 'propertyGridid',
					enableColLock : true,
				...
				var parameterStore = new Ext.data.GroupingStore({
					proxy : new Ext.data.HttpProxy({
						url : 'enlarging.do?code=queryEparameter'
				...
				var picturePanel = new Ext.Panel(
						{
							...
							region : 'center',
							// sm : sm2,
							// cm : hd_toolholder_coredrilling_shellCm,
							// tbar : tbar2,
							// bbar : bbar2,
							viewConfig : {
								forceFit : true
						...
						// view : new Ext.grid.GroupingView(),
						// plugins : [filtersHd_toolholder_coredrilling_shell],
						// stateful : true,
						// stateId : 'grid-hd_toolholder_coredrilling_shell'
						});
				...
				/*
				 * var hd_toolholder_coredrilling_shellStore = new
				 * Ext.data.Store( { proxy : new Ext.data.HttpProxy( { url :
				 * 'enlarging.do?code=queryHd_toolholder_coredrilling_shell4Manage'
				 * 
				 * }), reader : new Ext.data.JsonReader({ totalProperty :
				 * 'TOTALCOUNT', root : 'ROOT' }, [ { name : 'erpcode' }, { name :
				 * 'toolholdertype' }, { name : 'featurecode' } ]) });
				 */
				...
				var toolholderStore = new Ext.data.GroupingStore(
						{
							proxy : new Ext.data.HttpProxy(
									{
										url : 'enlarging.do?code=queryHd_module_coredrilling_shell4Manage'
									}),
							reader : new Ext.data.JsonReader({
								totalProperty : 'TOTALCOUNT',
								root : 'ROOT'
				...
				var toolholderPanel = new Ext.grid.GridPanel({
					...
					/*
					 * , { header:'型号', dataIndex:'type', width:80, //
					 * hidden:true, sortable:true } , { header:'简图',
					 * dataIndex:'figure', width:80, hidden:true, sortable:true } , {
					 * header:'实物图', dataIndex:'figure2', width:80, hidden:true,
					 * sortable:true } , { header:'品牌', dataIndex:'brandid',
					 * width:80, // hidden:true, sortable:true } , {
					 * header:'切削类型', dataIndex:'cuttype', width:80, //
					 * hidden:true, sortable:true } , { header:'总长',
					 * dataIndex:'length', width:80, // hidden:true,
					 * sortable:true } , { header:'钻孔深度', dataIndex:'depth',
					 * width:80, hidden:true, sortable:true } , { header:'接口尺寸',
					 * dataIndex:'portsize', width:80, hidden:true,
					 * sortable:true } , { header:'MCC', dataIndex:'mcc',
					 * width:80, // hidden:true, sortable:true } , {
					 * header:'底孔径', dataIndex:'porediamete', width:80,
					 * hidden:true, sortable:true } , { header:'冷却类型',
					 * dataIndex:'coolingtype', width:80, // hidden:true,
					 * sortable:true } , { header:'特点', dataIndex:'description',
					 * width:80, hidden:true, sortable:true } , { header:'备注信息',
					 * dataIndex:'remark', width:80, hidden:true, sortable:true } , {
					 * header:'附件信息', dataIndex:'aname', width:80, hidden:true,
					 * sortable:true }
					 */
					]),
					// tbar : tbar2,
				...
				var pictureTabs = new Ext.TabPanel(
						{
							...
							// width : document.body.clientWidth / 4,
							buttonAlign : 'right',
							items : [ picturePanel, sizePanel, toolholderPanel,
									sapPanel, cuttingparameter ],
							listeners : {
								tabchange : function(tabpanel, tab) {
									if (tab.getId() == 'toolholderPanelid') {
										toolholderStore.load({
											params : {
										...
										handler : function() {
											...
											if (index <= 0) {
												...
												propertyGrid
														.setSource({
															...
															'刀体类型' : record.data.hdtooltype,
															'齿数' : record.data.toothnumber,
															'底孔直径' : record.data.porediamete,
											...
											if (pictureTabs.getActiveTab()
												...
												toolholderStore
													...
													.getActiveTab().getItemId() == 'picturePanelid') {
												var newsrc = checkImagePath(record.data.figure);
												Ext.getCmp('boxid').getEl().dom.src = newsrc;
											} else if (pictureTabs
													.getActiveTab().getItemId() == 'sizePanelid') {

												var newsrc = checkImagePath(record.data.figure2);
												Ext.getCmp('boxid2').getEl().dom.src = newsrc;

											} else if (pictureTabs
													.getActiveTab().getItemId() == 'sapPanelid') {
												sapStore
														.load({
															params : {
																sapcode : record.data.sapcode
			...
			function printTask3() {
				...
				Ext.Ajax.request({
					...
					url : 'enlarging.do?code=printTask3',
