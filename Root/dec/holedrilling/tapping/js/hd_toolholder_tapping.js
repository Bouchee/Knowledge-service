...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'hd_toolholder_tapping.do?code=queryHd_toolholder_tapping4Manage'
			...
			var brandStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'hd_toolholder_tapping.do?code=queryBrand4Hd_toolholder_tapping'
				}),
				reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT',
							root : 'ROOT'
								...
								items : [
{
	...
	emptyText : '请输入总长...'}]
			...
			var grid = new Ext.grid.GridPanel({
						title		: '<span class="commoncss">丝锥</span>',
						header		: false,
						height		: 500,
						autoScroll	: true,
						frame		: true,
						...
						plugins		: [filters],
						stateful	: sf,
						stateId		: 'grid-hd_toolholder_tapping'
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							url : 'hd_toolholder_tapping.do?code=deleteHd_toolholder_tapping',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url : 'hd_toolholder_tapping.do?code=saveHd_toolholder_tapping',
							waitTitle : '提示',
							method : 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url : 'hd_toolholder_tapping.do?code=updateHd_toolholder_tapping',
							waitTitle : '提示',
							method : 'POST',
							submitEmptyText : false,
							waitMsg : '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'hd_toolholder_tapping.do?code=importHd_toolholder_tappingFromExcel',
															waitTitle : '提示',
															method : 'POST',
