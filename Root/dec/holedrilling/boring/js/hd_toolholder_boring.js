...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_toolholder_boring.do?code=queryHd_toolholder_boring4Manage'
	...
	var brandStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_toolholder_boring.do?code=queryBrand4Hd_toolholder_boring'
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
	...
	var grid = new Ext.grid.GridPanel({
		...
		plugins : [filters],
		stateful : sf,
		stateId : 'grid-hd_toolholder_boring'
			/*
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'hd_toolholder_boring.do?code=deleteHd_toolholder_boring',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
					url : 'hd_toolholder_boring.do?code=saveHd_toolholder_boring',
					waitTitle : '提示',
					method : 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
					url : 'hd_toolholder_boring.do?code=updateHd_toolholder_boring',
					waitTitle : '提示',
					method : 'POST',
					submitEmptyText : false,
					waitMsg : '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_toolholder_boring.do?code=importHd_toolholder_boringFromExcel',
													waitTitle : '提示',
													method : 'POST',
