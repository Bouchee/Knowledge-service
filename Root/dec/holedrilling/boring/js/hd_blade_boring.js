...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_blade_boring.do?code=queryHd_blade_boring4Manage'
	...
	var brandStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_blade_boring.do?code=queryBrand4Hd_blade_boring'
		}),
		reader : new Ext.data.JsonReader({
			totalProperty : 'TOTALCOUNT',
			root : 'ROOT'
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'hd_blade_boring.do?code=deleteHd_blade_boring',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
			url : 'hd_blade_boring.do?code=saveHd_blade_boring',
			waitTitle : '提示',
			method : 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
			url : 'hd_blade_boring.do?code=updateHd_blade_boring',
			waitTitle : '提示',
			method : 'POST',
			submitEmptyText : false,
			waitMsg : '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_blade_boring.do?code=importHd_blade_boringFromExcel',
													waitTitle : '提示',
													method : 'POST',
