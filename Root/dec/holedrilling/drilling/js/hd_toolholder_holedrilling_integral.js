	...
	sortable : true
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'hd_toolholder_holedrilling_integral.do?code=queryHd_toolholder_holedrilling_integral4Manage'
...
},
{
	...
	type:'string'
			...
			var brandStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'hd_toolholder_holedrilling_integral.do?code=queryBrand4Hd_toolholder_poledrilling'
				}),
				reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT',
							root : 'ROOT'
...
emptyText : '请输入系数K1...'}
			...
			var grid = new Ext.grid.GridPanel({
						title		: '<span class="commoncss">麻花钻</span>',
						header		: false,
						height		: 500,
						autoScroll	: true,
						frame		: true,
						...
						plugins		: [filters],
						stateful	: sf,
						stateId		: 'grid-hd_toolholder_holedrilling_integral'
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							url : 'hd_toolholder_holedrilling_integral.do?code=deleteHd_toolholder_holedrilling_integral',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url : 'hd_toolholder_holedrilling_integral.do?code=saveHd_toolholder_holedrilling_integral',
							waitTitle : '提示',
							method : 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url : 'hd_toolholder_holedrilling_integral.do?code=updateHd_toolholder_holedrilling_integral',
							waitTitle : '提示',
							method : 'POST',
							submitEmptyText : false,
							waitMsg : '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'hd_toolholder_holedrilling_integral.do?code=importHd_toolholder_holedrilling_integralFromExcel',
															waitTitle : '提示',
															method : 'POST',
