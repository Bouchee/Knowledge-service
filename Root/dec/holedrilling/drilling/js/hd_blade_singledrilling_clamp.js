...
Ext
		.onReady(function() {
			...
			var store = new Ext.data.GroupingStore(
					{
						proxy : new Ext.data.HttpProxy(
								{
									url : 'hd_blade_singledrilling_clamp.do?code=queryHd_blade_singledrilling_clamp4Manage'
								// queryHd_blade_singledrilling_clamp4Manage是在Hd_blade_singledrilling_clampAction中的方法queryHd_blade_singledrilling_clamp4Manage;
								}),
				...
				items : [ {
					...
					handler : function() {
						importExcelData();
							 ...
							 * 'page_edit_1Icon', id : 'readmtperformance',
							 * handler : function() { readPdf(); } }, '-',
							 */{
					xtype : 'textfield',
					id : 'inputname',
					name : 'inputname',
			...
			var brandStore = new Ext.data.Store(
					{
						proxy : new Ext.data.HttpProxy(
								{
									url : 'hd_blade_singledrilling_clamp.do?code=queryBrand4Hd_blade_singledrilling_clamp'
								}),
						reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT',
							root : 'ROOT'
			...
			var hd_blade_singledrilling_clampmaterialStore = new Ext.data.Store(
					{
						proxy : new Ext.data.HttpProxy(
								{
									url : 'hd_blade_singledrilling_clamp.do?code=queryMaterial4Hd_blade_singledrilling_clamp'
								}),
						reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT',
							root : 'ROOT'
			...
			hd_blade_singledrilling_clampmaterialStore.load(); // 如果mode :
			// 'local',时候才需要手动load();

			var hd_blade_singledrilling_clampmaterialCombo = new Ext.form.ComboBox(
					{
						fieldLabel : '刀片材质',
						...
						emptyText : '请选择刀片材质...',
						triggerAction : 'all',
						store : hd_blade_singledrilling_clampmaterialStore,
						displayField : 'text',
						valueField : 'value',
						loadingText : '正在加载数据...',
						mode : 'local', // 数据会自动读取,如果设置为local又调用了store.load()则会读取2次；也可以将其设置为local，然后通过store.load()方法来读取
						...
						listeners : {
							focus : function() {
								// hd_blade_singledrilling_clampmaterialStore.load();
								setTimeout(
										'Ext.getCmp("materialidid").expand();',
										1);
							}
						}
					});
			...
			// 切削液
			/*
			 * var categoryStore = new Ext.data.Store({ proxy : new
			 * Ext.data.HttpProxy({ url :
			 * 'machinetool.do?code=queryCategory4Machinetool' }), reader : new
			 * Ext.data.JsonReader({ totalProperty : 'TOTALCOUNT', root : 'ROOT' }, [{
			 * name : 'value' }, { name : 'text' }]), listeners : { load :
			 * function(store) { store.insert(0, new Ext.data.Record({ value :
			 * 'null', text : '暂不选择(置空)...' })); } }
			 * 
			 * }); categoryStore.load();
			 */
			 ...
			 * hiddenName : 'categoryid', id : 'categoryidid', emptyText :
			 * '请选择切削液...', cls : 'combobox-window-class', triggerAction :
			 * 'all', store : categoryStore, displayField : 'text', valueField :
			 * 'value', loadingText : '正在加载数据...', mode : 'local',
			 * forceSelection : true, // pageSize : cln, // minListWidth : 270,
			 * plugins : [new QM.plugin.PinyinFilter], resizable : true, anchor :
			 * '90%', listeners : { focus : function() { //
			 * categoryStore.load();
									...
									items : [
											{
												...
												items : [
														...
														hd_blade_singledrilling_clampmaterialCombo ]
											}, {
												columnWidth : .5,
												layout : 'form',
												defaultType : 'textfield',
			...
			var addItem = new Ext.Window(
					{
						...
						layout : 'fit',
						width : 800,
						height : Ext.isChrome ? 377 : (Ext.isFirefox ? 373
								: 373),
			...
			var grid = new Ext.grid.GridPanel({
				title : '<span class="commoncss">单刀片机夹钻刀片</span>',
				header : false,
				height : 500,
				autoScroll : true,
				frame : true,
				...
				// plugins : [ filters ],
				stateful : sf,
				stateId : 'grid-hd_blade_singledrilling_clamp'
			/*
			...
			function editInit() {
				...
				// 0);
				imageshowWin = imageshow2(record.data.figure,
						record.data.figure2, addItem, 0);
				if (imageshowWin != null) {
					imageshowWin.show();
				} else {
					infoMsg('该条目还没有添加图片!');
			...
			// 查看性能卡
			/*
			 * function readPdf() { var record =
			 * grid.getSelectionModel().getSelected(); var records =
			 * grid.getSelectionModel().getSelections(); if (records.length ==
			 * 0) { infoMsg('您没有选中任何条目，请先选中要修改的项目！'); return; } else if
			 ...
			 * false); }
			 * 
			 * var path1 = record.data.mtperformance; var path2 =
			 * record.data.figure;
			 * 
			 ...
			 * '\\resource\\image\\ext\\null.gif'; } var readWin = new
			 * Ext.Window({ title : '单刀片机夹钻刀片性能卡及简图', width : 800, modal : true,
			 * height : 500, closeAction : 'close', maximizable : true, layout :
			 * 'fit', items : [new Ext.TabPanel({ region : 'center', activeTab :
			 * 0, width : '100%', header : false, frame : true, items : [{ xtype :
			 * 'box', title : '性能卡', autoEl : { tag : 'iframe', style : 'height:
			 * 100%; width: 100%', src : webContext + path1 } }, { xtype :
			 * 'box', title : '单刀片机夹钻刀片简图', autoEl : { tag : 'iframe', style :
			 * 'height: 100%; width: 100%', src : webContext + path2 } }] })]
			 * 
			 * });
			 * 
			 * readWin.show(); }
						...
						.confirm(
								...
								function(btn, text) {
									if (btn == 'yes') {
										...
										Ext.Ajax
												.request({
													url : 'hd_blade_singledrilling_clamp.do?code=deleteHd_blade_singledrilling_clamp',
			...
			function submitForm() {
				...
				addItemForm.form
						.submit({
							url : 'hd_blade_singledrilling_clamp.do?code=saveHd_blade_singledrilling_clamp',
							waitTitle : '提示',
							method : 'POST',
			...
			function updateForm() {
				...
				addItemForm.form
						.submit({
							url : 'hd_blade_singledrilling_clamp.do?code=updateHd_blade_singledrilling_clamp',
							waitTitle : '提示',
							method : 'POST',
							submitEmptyText : false,
							waitMsg : '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'hd_blade_singledrilling_clamp.do?code=importHd_blade_singledrilling_clampFromExcel',
															waitTitle : '提示',
															method : 'POST',
