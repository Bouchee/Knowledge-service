...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_cutterbar_combineddrilling.do?code=queryHd_cutterbar_combineddrilling4Manage'
	...
	var brandStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_cutterbar_combineddrilling.do?code=queryBrand4Hd_cutterbar_combineddrilling'
		}),
		reader : new Ext.data.JsonReader({
			totalProperty : 'TOTALCOUNT',
			root : 'ROOT'
	...
	var grid = new Ext.grid.GridPanel({
		...
		plugins : [ filters ],
		stateful : sf,
		stateId : 'grid-hd_cutterbar_combineddrilling'
	/*
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'hd_cutterbar_combineddrilling.do?code=deleteHd_cutterbar_combineddrilling',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
			url : 'hd_cutterbar_combineddrilling.do?code=saveHd_cutterbar_combineddrilling',
			waitTitle : '提示',
			method : 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
			url : 'hd_cutterbar_combineddrilling.do?code=updateHd_cutterbar_combineddrilling',
			waitTitle : '提示',
			method : 'POST',
			submitEmptyText : false,
			waitMsg : '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_cutterbar_combineddrilling.do?code=importHd_cutterbar_combineddrillingFromExcel',
													waitTitle : '提示',
													method : 'POST',
