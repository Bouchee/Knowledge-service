...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_toolholder_combineddrilling.do?code=queryHd_toolholder_combineddrilling4Manage'
				// queryHd_toolholder_combineddrilling4Manage是在Hd_toolholder_combineddrillingAction中的方法queryHd_toolholder_combineddrilling4Manage;
		}),
	...
	var brandStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_toolholder_combineddrilling.do?code=queryBrand4Hd_toolholder_combineddrilling'
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
	...
	var grid = new Ext.grid.GridPanel({
		...
		plugins : [filters],
		stateful : sf,
		stateId : 'grid-hd_toolholder_combineddrilling'
			/*
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'hd_toolholder_combineddrilling.do?code=deleteHd_toolholder_combineddrilling',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
					url : 'hd_toolholder_combineddrilling.do?code=saveHd_toolholder_combineddrilling',
					waitTitle : '提示',
					method : 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
					url : 'hd_toolholder_combineddrilling.do?code=updateHd_toolholder_combineddrilling',
					waitTitle : '提示',
					method : 'POST',
					submitEmptyText : false,
					waitMsg : '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_toolholder_combineddrilling.do?code=importHd_toolholder_combineddrillingFromExcel',
													waitTitle : '提示',
													method : 'POST',
