...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_blade_combined_centredrilling.do?code=queryHd_blade_combined_centredrilling4Manage'
	...
	var brandStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_blade_combined_centredrilling.do?code=queryBrand4Hd_blade_combined_centredrilling'
		}),
		reader : new Ext.data.JsonReader({
			totalProperty : 'TOTALCOUNT',
			root : 'ROOT'
	...
	var grid = new Ext.grid.GridPanel({
		...
		plugins : [ filters ],
		stateful : sf,
		stateId : 'grid-hd_blade_combined_centredrilling'
	/*
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'hd_blade_combined_centredrilling.do?code=deleteHd_blade_combined_centredrilling',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
			url : 'hd_blade_combined_centredrilling.do?code=saveHd_blade_combined_centredrilling',
			waitTitle : '提示',
			method : 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
			url : 'hd_blade_combined_centredrilling.do?code=updateHd_blade_combined_centredrilling',
			waitTitle : '提示',
			method : 'POST',
			submitEmptyText : false,
			waitMsg : '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_blade_combined_centredrilling.do?code=importHd_blade_combined_centredrillingFromExcel',
													waitTitle : '提示',
													method : 'POST',
