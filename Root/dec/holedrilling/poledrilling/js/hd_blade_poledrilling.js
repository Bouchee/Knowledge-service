...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
				proxy	: new Ext.data.HttpProxy({
					url	: 'hd_blade_poledrilling.do?code=queryHd_blade_poledrilling4Manage'
	...
	var brandStore = new Ext.data.Store({
				proxy		: new Ext.data.HttpProxy({
							url	: 'hd_blade_poledrilling.do?code=queryBrand4Hd_blade_poledrilling'
						}),
				reader		: new Ext.data.JsonReader({
							totalProperty	: 'TOTALCOUNT',
							root			: 'ROOT'
	...
	var grid = new Ext.grid.GridPanel({
		title		: '<span class="commoncss">浅孔钻刀片</span>',
		header		: false,
		height		: 500,
		autoScroll	: true,
		frame		: true,
		...
		plugins		: [filters],
		stateful	: sf,
		stateId		: 'grid-milling_blade_clamp'
			/*
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url		: 'hd_blade_poledrilling.do?code=deleteHd_blade_poledrilling',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
					url				: 'hd_blade_poledrilling.do?code=saveHd_blade_poledrilling',
					waitTitle		: '提示',
					method			: 'POST',
	...
	function updateForm() {
		if (!addItemForm.getForm().isValid()) {
			...
			checkUploadImage('figureid');
		checkUploadImage('figure2id');

		addItemForm.form.submit({
					url				: 'hd_blade_poledrilling.do?code=updateHd_blade_poledrilling',
					waitTitle		: '提示',
					method			: 'POST',
					submitEmptyText	: false,
					waitMsg			: '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_blade_poledrilling.do?code=importHd_blade_poledrillingFromExcel',
													waitTitle : '提示',
													method : 'POST',
