...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
							url	: 'hd_toolholder_poledrilling.do?code=queryHd_toolholder_poledrilling4Manage'
			...
			var brandStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'hd_toolholder_poledrilling.do?code=queryBrand4Hd_toolholder_poledrilling'
				}),
				reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT',
							root : 'ROOT'
...
emptyText : '请输入系数K1...'},
{
	...
	emptyText : '请输入公称直径'}]
			...
			var grid = new Ext.grid.GridPanel({
						title		: '<span class="commoncss">浅孔钻</span>',
						header		: false,
						height		: 500,
						autoScroll	: true,
						frame		: true,
						...
						plugins		: [filters],
						stateful	: sf,
						stateId		: 'grid-hd_toolholder_poledrilling'
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							url : 'hd_toolholder_poledrilling.do?code=deleteHd_toolholder_poledrilling',
			...
			function submitForm() {
				...
				addItemForm.form.submit({
							url : 'hd_toolholder_poledrilling.do?code=saveHd_toolholder_poledrilling',
							waitTitle : '提示',
							method : 'POST',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url : 'hd_toolholder_poledrilling.do?code=updateHd_toolholder_poledrilling',
							waitTitle : '提示',
							method : 'POST',
							submitEmptyText : false,
							waitMsg : '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'hd_toolholder_poledrilling.do?code=importHd_toolholder_poledrillingFromExcel',
															waitTitle : '提示',
															method : 'POST',
