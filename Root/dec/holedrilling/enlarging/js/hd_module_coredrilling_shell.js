...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_module_coredrilling_shell.do?code=queryHd_module_coredrilling_shell4Manage'
				// queryHd_blade_boring4Manage是在Hd_blade_boringAction中的方法queryHd_blade_boring4Manage;
		}),
	...
	var brandStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'hd_module_coredrilling_shell.do?code=queryBrand4Hd_module_coredrilling_shell'
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
				...
				items : [{
					...
					items : [{
								...
								items : [{
									 ...
									 * , { fieldLabel : ' 编号', name :
									 * 'toolholderid', // labelStyle : micolor, //
									 * allowBlank : false, emptyText :
									 * '请输入刀片名称...' }
									 */
								, {
									fieldLabel : 'SAP编号',
	...
	var grid = new Ext.grid.GridPanel({
		title : '<span class="commoncss">套式扩孔钻模块</span>',
		header : false,
		height : 500,
		autoScroll : true,
		frame : true,
		...
		plugins : [filters],
		stateful : sf,
		stateId : 'grid-hd_module_coredrilling_shell'
			/*
	 ...
	 * title : '机夹刀具刀片性能卡及简图', width : 800, modal : true, height : 500,
	 ...
	 * path1 } }, { xtype : 'box', title : '机夹刀具刀片简图', autoEl : { tag :
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request({
					url : 'hd_module_coredrilling_shell.do?code=deleteHd_module_coredrilling_shell',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
			url : 'hd_module_coredrilling_shell.do?code=saveHd_module_coredrilling_shell',
			waitTitle : '提示',
			method : 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
			url : 'hd_module_coredrilling_shell.do?code=updateHd_module_coredrilling_shell',
			waitTitle : '提示',
			method : 'POST',
			submitEmptyText : false,
			waitMsg : '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'hd_module_coredrilling_shell.do?code=importHd_module_coredrilling_shellFromExcel',
													waitTitle : '提示',
													method : 'POST',
