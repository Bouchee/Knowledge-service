...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
									url	: 'groove.do?code=queryGroove4Manage'
								}),
			...
			var accuracyStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'groove.do?code=queryAccuracy4Groove'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			var brandStore = new Ext.data.Store({
						proxy		: new Ext.data.HttpProxy({
									url	: 'groove.do?code=queryBrand4Groove'
								}),
						reader		: new Ext.data.JsonReader({
									totalProperty	: 'TOTALCOUNT',
									root			: 'ROOT'
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'groove.do?code=deleteGroove',
			...
			function updateForm() {
				...
				addItemForm.form.submit({
							url				: 'groove.do?code=updateGroove',
							waitTitle		: '提示',
							method			: 'POST',
							submitEmptyText	: false,
							waitMsg			: '正在处理数据,请稍候...',
							...
							items : [
									{
										...
										listeners : {
											change : function(tf, newValue,
												...
												importpanel.form
														.submit({
															url : 'groove.do?code=importGrooveFromExcel',
															waitTitle : '提示',
															method : 'POST',
