 ...
 * @since 2013-01-01
 */
// 仿形铣 //Templetmilling //templetmilling
// 公称直径//Nominaldiameter//nominaldiameter
// 切削深度//Interferencedepth//interferencedepth
// 刀尖圆弧半径//Noseradius//noseradius
// 刃部锥角//Coneangle//coneangle
...
*/		sortable : true
	...
	var milling_toolholder_integralStore = new Ext.data.GroupingStore({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryMilling_toolholder_integral4Manage'
				// queryMilling_toolholder_integral4Manage是在TempletmillingAction中的方法queryMilling_toolholder_integral4Manage;
		}),
	...
	var milling_toolholder_clampStore = new Ext.data.GroupingStore({
				proxy : new Ext.data.HttpProxy({
					url : 'templetmilling.do?code=queryMilling_toolholder_clamp4Manage'
						// queryMilling_toolholder_clamp4Manage是在TempletmillingAction中的方法queryMilling_toolholder_clamp4Manage;
				}),
				reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT',
							root : 'ROOT'
	...
	var milling_blade_clampStore = new Ext.data.GroupingStore({
				proxy : new Ext.data.HttpProxy({
					url : 'templetmilling.do?code=queryMilling_blade_clamp4Manage'
	 ...
	 * 
	 * if (records.length == 0) { milling_toolholder_clampStoreLoad; } else { if
	...
	/*
	 * integralStore.load({ params : { start : 0, limit : bbar.pageSize } });
	 */
	...
	var nominaldiameterStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryNominaldiameterInMilling_toolholder_integral'
				// queryNominaldiameterInMilling_toolholder_integral是在TempletmillingAction中的方法queryNominaldiameterInMilling_toolholder_integral;
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
	...
	//提示框
	var gongchengzhijinDisplayWin2 = new Ext.Window(
			{
				//title : '选择助手',
				id:'gongchengzhijinDisplayWin2id',
				...
				closable:false,
			     ...
			     items: [ {
						...
						autoEl : {
							...
							src : 'dec/combo/alter/new/fangxingxi_zhengti_gongchengzhijin.png'
						}
					} 
	...
	var nominaldiameterCombo = new Ext.form.ComboBox({
				...
				listeners : {
					...
					expand:function(){gongchengzhijinDisplayWin2.show()},
					collapse:function(){gongchengzhijinDisplayWin2.hide()},
					...
					specialkey : function(c, e) {
						if (e.getKey() == e.ESC) {
							if (parent.grf == true) {
								...
								nominaldiameterCombo2.setValue(null);
		...
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryNameInMilling_toolholder_integral'
	...
	var noseradiusStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryNoseradiusInMilling_toolholder_integral'
				// queryMachinetool4Manage是在TempletmillingAction中的方法queryMachinetool4Manage;
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
	...
	//提示框
	var daojianyuanhuDisplayWin2 = new Ext.Window(
			{
				//title : '选择助手',
				id:'daojianyuanhuDisplayWin2id',
				x:((document.body.clientWidth)/numlign-divnum)*4+285,
				y:(document.body.clientWidth)*(1/11),
				width : 200,
				//modal : true,
				height : 200,
				...
				closable:false,
			     ...
			     items: [ {
						...
						autoEl : {
							tag : 'img',
							style : 'width:100%;height:100%;' ,
							src : 'dec/combo/alter/new/fangxingxi_zhengti_daojianyuanhubanjin.png'
						}
					} 
	...
	var noseradiusCombo = new Ext.form.ComboBox({
				...
				listeners : {
					...
					expand:function(){daojianyuanhuDisplayWin2.show()},
					collapse:function(){daojianyuanhuDisplayWin2.hide()},
					select : function() {
						/* noseradiusCombo2.setValue(noseradiusCombo.getValue()); */
						refreshMilling_toolholder_integral();
					},
					specialkey : function(c, e) {
						if (e.getKey() == e.ESC) {
	...
	var celengthStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryCelengthInMilling_toolholder_integral'
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
	...
	//提示框
	var qiexiaoshenduDisplayWin2 = new Ext.Window(
			{
				//title : '选择助手',
				id:'qiexiaoshenduDisplayWin2id',
				x:((document.body.clientWidth)/numlign-divnum)*5+345,
				y:(document.body.clientWidth)*(1/11),
				width : 200,
				//modal : true,
				height : 200,
				...
				closable:false,
			     ...
			     items: [ {
						...
						autoEl : {
							tag : 'img',
							style : 'width:100%;height:100%;' ,
							src : 'dec/combo/alter/new/fangxingxi_zhengti_qiexiaoshendu.png'
						}
					} 
	...
	var celengthCombo = new Ext.form.ComboBox({
				...
				listeners : {
					...
					expand:function(){qiexiaoshenduDisplayWin2.show()},
					collapse:function(){qiexiaoshenduDisplayWin2.hide()},
					select : function(c, re, index) {
	...
	// 刃部锥角数据
	var coneangleStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryConeangleInMilling_toolholder_integral'
				// queryConeangleInMilling_toolholder_integral是在TempletmillingAction中的方法queryConeangleInMilling_toolholder_integral;
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
	...
	// 刃部锥角下拉框
	var coneangleCombo = new Ext.form.ComboBox({
				fieldLabel : '刃部锥角',
				hiddenName : 'coneangle',
				id : 'coneangleid',
				emptyText : '请选择...',
				triggerAction : 'all',
				store : coneangleStore,
				displayField : 'value',
				valueField : 'value',
				selectOnFocus : true,
				loadingText : '正在加载数据...',
				...
				listeners : {
					focus : function() {
						// noseradiusStore.load();
						setTimeout('Ext.getCmp("coneangleid").expand();', 1);
					},
					select : function() {
						// coneangleCombo2.setValue(coneangleCombo.getValue());
						refreshMilling_toolholder_integral();
					},
					specialkey : function(c, e) {
						if (e.getKey() == e.ESC) {
							if (parent.grf == true) {
								c.reset();
								/*
								 * coneangleCombo2.reset();
								 * coneangleCombo2.setValue('');
								 */
							}
							refreshMilling_toolholder_integralForce();

						}if (e.getKey() == e.ENTER) {
						c.setValue(Ext.get('coneangleid').dom.value) ;
							refreshMilling_toolholder_integralForce();
						}
					}
				}
	...
	var nominaldiameterStore2 = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryNominaldiameterInMilling_toolholder_clamp'
				// queryNominaldiameterInMilling_toolholder_clamp是在TempletmillingAction中的方法queryNominaldiameterInMilling_toolholder_clamp;
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
				...
				closable:false,
			     ...
			     items: [ {
						...
						autoEl : {
							tag : 'img',
							style : 'width:100%;height:100%;' ,
							src : 'dec/combo/alter/new/fangxingxi_jijia_gongchengzhijin.png'
						}
					} 
	...
	var nameStore2 = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryNameInMilling_toolholder_clamp'
				// queryMachinetool4Manage是在MachinetoolAction中的方法queryMachinetool4Manage;
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
	...
	var interferencedepthStore2 = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'templetmilling.do?code=queryInterferencedepthInMilling_toolholder_clamp'
		}),
		reader : new Ext.data.JsonReader({
					totalProperty : 'TOTALCOUNT',
					root : 'ROOT'
				...
				closable:false,
			     ...
			     items: [ {
						...
						autoEl : {
							tag : 'img',
							style : 'width:100%;height:100%;' ,
							src : 'dec/combo/alter/new/fangxingxi_jijia_qiexiaoshendu.png'
						}
					} 
	...
	var interferencedepthCombo2 = new Ext.form.ComboBox({
				...
				listeners : {
					...
					select : function(c, re, index) {
						// interferencedepthCombo.setValue(interferencedepthCombo2.getValue());
	...
	var noseradiusStore3 = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'templetmilling.do?code=queryNoseradiusInMilling_blade_clamp'
						// queryNoseradiusInMilling_blade_clamp是在TempletmillingAction中的方法queryNoseradiusInMilling_blade_clamp;
				}),
				reader : new Ext.data.JsonReader({
							totalProperty : 'TOTALCOUNT',
							root : 'ROOT'
				...
				closable:false,
			     ...
			     items: [ {
						...
						autoEl : {
							tag : 'img',
							style : 'width:100%;height:100%;' ,
							src : 'dec/combo/alter/new/fangxingxi_jijia_daojianyuanhubanjin.png'
						}
					} 
	...
	/*
	 * // 刀尖圆弧半径数据 var noseradiusStore2 = new Ext.data.Store({ proxy : new
	 * Ext.data.HttpProxy({ url :
	 * 'templetmilling.do?code=queryNoseradiusInMilling_toolholder_integral' //
	 * queryMachinetool4Manage是在TempletmillingAction中的方法queryMachinetool4Manage;
	 * }), reader : new Ext.data.JsonReader({ totalProperty : 'TOTALCOUNT', root :
	 * 'ROOT' }, [{ name : 'value' }, { name : 'value' }]) });
	 * noseradiusStore2.load();
	 */
	/*
	 * // 刀尖圆弧半径下拉框 var noseradiusCombo2 = new Ext.form.ComboBox({ fieldLabel :
	 * '刀尖圆弧半径', hiddenName : 'noseradius', id : 'noseradiusid2', emptyText :
	 * '请选择...', triggerAction : 'all', store : noseradiusStore2, displayField :
	 ...
	 * true, width : document.body.clientWidth / numlign - divnum, // listWidth :
	 * noseradiusArr[0] * noseradiuspp + 25,
	 * 
	 * listeners : { focus : function() { // noseradiusStore.load();
	 * setTimeout('Ext.getCmp("noseradiusid2").expand();', 1); }, select :
	 * function() { noseradiusCombo.setValue(noseradiusCombo2.getValue());
	 * refreshMilling_toolholder_clamp(); }, specialkey : function(c, e) { if
	 * (e.getKey() == e.ENTER) { if (parent.grf == true) { c.reset();
	 * nominaldiameterCombo.reset(); nominaldiameterCombo.setValue(''); }
	 * refreshMilling_toolholder_clampForce(); } } } });
	 */
	/*
	 * // 刃部锥角数据 var coneangleStore2 = new Ext.data.Store({ proxy : new
	 * Ext.data.HttpProxy({ url :
	 * 'templetmilling.do?code=queryConeangleInMilling_toolholder_integral' //
	 * queryConeangleInMilling_toolholder_integral是在TempletmillingAction中的方法queryConeangleInMilling_toolholder_integral;
	 * }), reader : new Ext.data.JsonReader({ totalProperty : 'TOTALCOUNT', root :
	 * 'ROOT' }, [{ name : 'value' }, { name : 'value' }]) });
	 * noseradiusStore2.load();
	 */
	/*
	 * // 刃部锥角下拉框 var coneangleCombo2 = new Ext.form.ComboBox({ fieldLabel :
	 * '刃部锥角', hiddenName : 'coneangle', id : 'coneangleid2', emptyText :
	 * '请选择...', triggerAction : 'all', store : noseradiusStore2, displayField :
	 * 'text', valueField : 'value', selectOnFocus : true, loadingText :
	 * '正在加载数据...', mode : 'local', forceSelection : true, // pageSize : cln, //
	 * minListWidth : 270, plugins : [new QM.plugin.PinyinFilter], resizable :
	 ...
	 * 
	 * listeners : { focus : function() { // noseradiusStore.load();
	 * setTimeout('Ext.getCmp("coneangleid2").expand();', 1); }, select :
	 * function() { coneangleCombo.setValue(noseradiusCombo2.getValue());
	 * refreshMilling_toolholder_clamp(); }, specialkey : function(c, e) { if
	 * (e.getKey() == e.ENTER) { if (parent.grf == true) { c.reset();
	 * coneangleCombo.reset(); coneangleCombo.setValue(''); }
	 * refreshMilling_toolholder_clampForce(); } } } });
	 */
	// 表格工具栏（整体刀体）
	var tbar = new Ext.Toolbar({// workpiecematerialCombo 
		...
		items : [new Ext.Toolbar.TextItem({
				...
				new Ext.Toolbar.TextItem({
							text : '刀尖圆弧半径',
							autoHeight : true
						}), noseradiusCombo, '-', new Ext.Toolbar.TextItem({
							text : '切削深度',
							autoHeight : true
	...
	var tbar2 = new Ext.Toolbar({// workpiecematerialCombo 
		...
		items : [new Ext.Toolbar.TextItem({
				...
				/*
				 * new Ext.Toolbar.TextItem({ text : '刀尖圆弧半径', autoHeight : true
				 * }), noseradiusCombo2, '-', new Ext.Toolbar.TextItem({ text :
				 * '刃部锥角', autoHeight : true }), coneangleCombo2, '->',
				 */{
					text : '重置',
					iconCls : 'tbar_synchronizeIcon',
					handler : function() {
	...
	grid3.on("contextmenu", function(e) {// cellcontextmenu //rowcontextmenu
				e.preventDefault();
			});
				...
				listeners : {
					tabchange : function(tabpanel, tab) {
						...
						if (tabpanel.getActiveTab().getId() == 'gridid') {// 激活了刀体grid;
							...
							// refreshMilling_toolholder_clampForce();
							milling_toolholder_clampStoreLoad();
							// } else if (tabpanel.getActiveTab().getId() ==
							// 'grid3id') {
							// milling_blade_clampStore.reload();
							// refreshMilling_blade_clampForce();
							milling_blade_clampStoreLoad();
	...
	function resetCombo() {
		 ...
		 * 
		 * mainangleCombo.reset();
		 * 
		 * shapeCombo2.setValue(''); //shapeCombo2.reset();
		 * reliefangleCombo2.setValue(''); //reliefangleCombo2.reset();
		 * 
		 * //milling_toolholder_integralStore.reload(); /* .load({ params : {
		 * workpieceisotype : '', accuracyid : '', shape : '', reliefangle : '',
		 * celength : '', noseradius : '' } });
		 */
	...
	// 更新
	/*
	 * function resetCombo3() { workpieceisotypeCombo3.setValue('');
	 * noseradiusCombo3.setValue('');
	 * 
	 * nominaldiameterCombo2.setValue(''); interferencedepthCombo2.setValue('');
	 * 
	 * workpieceisotypeCombo.setValue(''); nominaldiameterCombo.setValue('');
	 * noseradiusCombo.setValue(''); coneangleCombo.setValue('');
	 * 
	 * milling_blade_clampStoreLoad(); }
	 */
	...
	function propertyDisplay() {
		...
		var picturePanel = new Ext.Panel({
			...
			items : [{  id:'boxid',
				...
				// plugins : [filtersMilling_toolholder_clamp],
				// stateful : true,
				// stateId : 'grid-templetmilling-milling_toolholder_clamp'
			});
		...
		var parameterStore = new Ext.data.GroupingStore({
					proxy : new Ext.data.HttpProxy({
						url : 'templetmilling.do?code=queryIntegralParameter'
							// queryMilling_blade_clamp4Manage是在TempletmillingAction中的方法queryMilling_blade_clamp4Manage;
						}),
					reader : new Ext.data.JsonReader({
								totalProperty : 'TOTALCOUNT',
	...
	function propertyDisplay2() {
		 ...
		 * 'grid-templetmilling-milling_toolholder_clamp' } );
		 */
	...
	function propertyDisplay3() {
		 ...
		 * 'templetmilling.do?code=queryMilling_toolholder_integral4Manage' //
		 * queryMilling_toolholder_integral4Manage是在Milling_toolholder_integralAction中的方法queryMilling_toolholder_integral4Manage; } ),
		 * reader : new Ext.data.JsonReader({ totalProperty : 'TOTALCOUNT', root :
		...
		var parameterStore = new Ext.data.GroupingStore({
					proxy : new Ext.data.HttpProxy({
						url : 'templetmilling.do?code=queryParameter'
							// queryMilling_blade_clamp4Manage是在TempletmillingAction中的方法queryMilling_blade_clamp4Manage;
						}),
					reader : new Ext.data.JsonReader({
								totalProperty : 'TOTALCOUNT',
	...
	function milling_toolholder_integralStoreLoad() {
		milling_toolholder_integralStore.load({
			params : {
				...
				coneangle : coneangleCombo.getValue(),
				name : nameCombo.getValue()
			...
			milling_blade_clampStore.load({
						params : {
							...
							workpieceisotype : workpieceisotypeCombo3
									.getValue(),
							nominaldiameter : nominaldiameterCombo2.getValue(),
							interferencedepth : interferencedepthCombo2
									.getValue(),
							noseradius : noseradiusCombo3.getValue(),
	...
	function printTask() {
		...
		Ext.Ajax.request({
			...
			url : 'templetmilling.do?code=printTask',
	...
	function printTask2() {
		...
		Ext.Ajax.request({
			...
			url : 'templetmilling.do?code=printTask2',
	...
	function printTask3() {
		...
		Ext.Ajax.request({
			...
			url : 'templetmilling.do?code=printTask3',
