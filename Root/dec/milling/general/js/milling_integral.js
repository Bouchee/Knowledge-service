...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
				proxy	: new Ext.data.HttpProxy({
					url	: 'milling_integral.do?code=queryMilling_integral4Manage'
						// queryMilling_integral4Manage是在Milling_integralAction中的方法queryMilling_integral4Manage;
					}),
	...
	var brandStore = new Ext.data.Store({
				proxy		: new Ext.data.HttpProxy({
							url	: 'milling_integral.do?code=queryBrand4Milling_integral'
						}),
				reader		: new Ext.data.JsonReader({
							totalProperty	: 'TOTALCOUNT',
							root			: 'ROOT'
	...
	var addItem = new Ext.Window({
				...
				layout			: 'fit',
				width			: 800,
				height			: Ext.isChrome ? 477 : (Ext.isFirefox ? 473 : 473),
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url		: 'milling_integral.do?code=deleteMilling_integral',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
					url				: 'milling_integral.do?code=saveMilling_integral',
					waitTitle		: '提示',
					method			: 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
					url				: 'milling_integral.do?code=updateMilling_integral',
					waitTitle		: '提示',
					method			: 'POST',
					submitEmptyText	: false,
					waitMsg			: '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'milling_integral.do?code=importMilling_integralFromExcel',
													waitTitle : '提示',
													method : 'POST',
