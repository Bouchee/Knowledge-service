...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
				proxy	: new Ext.data.HttpProxy({
					url	: 'milling_blade_clamp.do?code=queryMilling_blade_clamp4Manage'
	...
	var brandStore = new Ext.data.Store({
				proxy		: new Ext.data.HttpProxy({
							url	: 'milling_blade_clamp.do?code=queryBrand4Milling_blade_clamp'
						}),
				reader		: new Ext.data.JsonReader({
							totalProperty	: 'TOTALCOUNT',
							root			: 'ROOT'
	...
	function editInit() {
		...
		Ext.getCmp('bladeid').setValue(record.get('bladeid'));
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url		: 'milling_blade_clamp.do?code=deleteMilling_blade_clamp',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
					url				: 'milling_blade_clamp.do?code=saveMilling_blade_clamp',
					waitTitle		: '提示',
					method			: 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
					url				: 'milling_blade_clamp.do?code=updateMilling_blade_clamp',
					waitTitle		: '提示',
					method			: 'POST',
					submitEmptyText	: false,
					waitMsg			: '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'milling_blade_clamp.do?code=importMilling_blade_clampFromExcel',
													waitTitle : '提示',
													method : 'POST',
