...
Ext.onReady(function() {
	...
	var store = new Ext.data.GroupingStore({
				proxy	: new Ext.data.HttpProxy({
					url	: 'milling_toolholder_clamp.do?code=queryMilling_toolholder_clamp4Manage'
	...
	var brandStore = new Ext.data.Store({
				proxy		: new Ext.data.HttpProxy({
							url	: 'milling_toolholder_clamp.do?code=queryBrand4Milling_toolholder_clamp'
						}),
				reader		: new Ext.data.JsonReader({
							totalProperty	: 'TOTALCOUNT',
							root			: 'ROOT'
	...
	var addItemForm = new Ext.form.FormPanel({
				...
				items		: [{
							...
							name		: 'featurecode',
							labelStyle	: fucolor,
							emptyText	: '请输入特征码...（注意格式特征码之间用‘|’隔开）'
						}, {
							fieldLabel	: '备注',
							// labelStyle : fucolor,
							allowBlank	: true,
	...
	var addItem = new Ext.Window({
				...
				layout			: 'fit',
				width			: 800,
				height			: Ext.isChrome ? 600 : (Ext.isFirefox ? 697 : 697),
	...
	var grid = new Ext.grid.GridPanel({
		...
		plugins		: [filters],
		stateful	: sf,
		stateId		: 'grid-milling_toolholder_clamp'
			/*
	 ...
	 * 
	 * if (path2 == null || path2 == '') { path2 = '\\resource\\image\\ext\\null.gif'; } var readWin = new Ext.Window({ title : '机夹刀具刀体性能卡及简图', width : 800, modal : true, height : 500, closeAction :
	 ...
	 * autoEl : { tag : 'iframe', style : 'height: 100%; width: 100%', src : webContext + path1 } }, { xtype : 'box', title : '机夹刀具刀体简图', autoEl : { tag : 'iframe', style : 'height: 100%; width:
	...
	function deleteInit() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中的记录吗', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url		: 'milling_toolholder_clamp.do?code=deleteMilling_toolholder_clamp',
	...
	function submitForm() {
		...
		addItemForm.form.submit({
					url				: 'milling_toolholder_clamp.do?code=saveMilling_toolholder_clamp',
					waitTitle		: '提示',
					method			: 'POST',
	...
	function updateForm() {
		...
		addItemForm.form.submit({
					url				: 'milling_toolholder_clamp.do?code=updateMilling_toolholder_clamp',
					waitTitle		: '提示',
					method			: 'POST',
					submitEmptyText	: false,
					waitMsg			: '正在处理数据,请稍候...',
					...
					items : [
							{
								...
								listeners : {
									change : function(tf, newValue,
										...
										importpanel.form
												.submit({
													url : 'milling_toolholder_clamp.do?code=importMilling_toolholder_clampFromExcel',
													waitTitle : '提示',
													method : 'POST',
