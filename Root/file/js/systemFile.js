...
Ext.onReady(function() {
			...
			var store = new Ext.data.GroupingStore({
						proxy	: new Ext.data.HttpProxy({
									url	: 'systemFile.do?code=queryFile4Manage'
								}),
			...
			var resultTpl = new Ext.XTemplate('<tpl for="."><div class="search-item">', '<h3>{remark}</h3>', '{title}', '</div></tpl>');
			...
			var tbar = new Ext.Toolbar({
						items	: [{
									 ...
									 * { xtype : 'combo', id : 'inputname', store : store, cls : 'search', //name : 'filetitle', displayField : 'title', emptyText : '文件标题', //loadingText:
									 * '正在加载数据，请稍候...', width : 150, //pageSize: cln, itemSelector : 'div.search-item', typeAhead : false, tpl : resultTpl, hideTrigger : true, enableKeyEvents : true, //
									 * 响应回车键 listeners : { specialkey : function(field, e) { if (e.getKey() == Ext.EventObject.ENTER) { queryInfo(); } }, select : function(record) {
			...
			grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
						...
						if (fieldName == 'download' && columnIndex == 2) {
							...
							downloadIframe.src = 'systemFile.do?code=downloadFile&fileid=' + fileid;
			...
			function deleteInit() {
				...
				Ext.Msg.confirm('请确认', '确认要删除选中的文件吗?', function(btn, text) {
							if (btn == 'yes') {
								showWaitMsg();
								Ext.Ajax.request({
											url		: 'systemFile.do?code=deleteSystemfile',
			...
			function uploadHttp() {
				var uploadHttpForm = new Ext.form.FormPanel({
							id			: 'firstForm',
							name		: 'firstForm',
				...
				var uploadWindow = new Ext.Window({
							...
							// pageY : document.body.clientHeight / 2 - 200 / 2,//// pageY : 20, // 页面定位X坐标
							// pageX : document.body.clientWidth / 2 - 500 / 2, // 页面定位Y坐标
							items			: [uploadHttpForm], // 嵌入的表单面板
							buttons			: [{ // 窗口底部按钮配置
								text	: '上传', // 按钮文本
				...
				function submitTheForm() {
					if (!uploadHttpForm.form.isValid()) {
						return;
					}
					if (mode == 'ftp') {
					...
					var requesturl = 'systemFile.do?code=doUpload';
					if (mode == 'ftp') {
						requesturl = 'systemFile.do?code=doUploadByFtp';
					}
					uploadHttpForm.form.submit({
								url			: requesturl,
								waitTitle	: '提示',
								method		: 'POST',
			...
			function uploadSwf() {
				...
				var swfuploadWindow = new Ext.Window({
							...
							items		: [{
										xtype					: 'uploadpanel',
										uploadUrl				: webContext + 'systemFile.do?code=doUploadByFlash',
			...
			function saveEditorData() {
				...
				// 提交到后台处理
				Ext.Ajax.request({
							url		: 'systemfile.do?code=saveEditorData',
