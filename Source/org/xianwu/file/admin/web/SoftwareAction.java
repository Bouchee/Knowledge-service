 ...
 * @see BaseAction
 */
public class SoftwareAction extends BizAction {

	private SoftwareService softwareService = (SoftwareService) getService("softwareService");
	private String filepath = "\\upload\\software\\";
	...
	public ActionForward init(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		return mapping.findForward("softwareView");
	}
	...
	public ActionForward querySoftware4Manage(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		CommonActionForm aForm = (CommonActionForm) form;
		Dto dto = aForm.getParamAsDto(request);
		List list = g4Reader.queryForPage("Software.querySoftware4Manage", dto);
		Integer countInteger = (Integer) g4Reader.queryForObject("Software.querySoftware4Manage4PageCount", dto);
		String jsonString = JsonHelper.encodeList2PageJson(list, countInteger, SystemConstants.FORMAT_Date);
	...
	public ActionForward doUpload(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// UserInfoVo userInfo =
		// WebUtils.getSessionContainer(request).getUserInfo();
		...
		inDto.put("username", userInfo.getUsername());
		inDto.put("ip", userInfo.getLoginIP());
		inDto.put("submittime", Utils.getCurrentTime());

		softwareService.insertSoftware(inDto);
	...
	public ActionForward downloadFile(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Dto fileDto = (BaseDto) g4Reader.queryForObject("Software.querySoftwareByKey", fileid);
	 ...
	 * @throws Exception
	 */
	public ActionForward deleteSoftware(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		CommonActionForm aForm = (CommonActionForm) form;
		Dto dto = aForm.getParamAsDto(request);
		...
		for (int i = 0; i < strChecked.length; i++) {
			...
			file.delete();
			softwareService.deleteSoftware(fileid);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ActionForward updateSoftware(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		CommonActionForm aForm = (CommonActionForm) form;
		Dto dto = aForm.getParamAsDto(request);
		String field = dto.getAsString("field");
		String value = dto.getAsString("value");
		dto.put(field, value);
		dto.put("fileid", dto.getAsString("fileid"));

		softwareService.updateSoftware(dto);
